...
6. **竞价成交量爆量股**（东方财富量比 `f50`；阈值 `auction_report.volume_ratio_spike`。量比不可用时整段 `DATA_MISSING`，**不用成交额冒充爆量**）
...
测试使用夹具行情，不依赖交易时段，覆盖评分分档、一股一链、3 日资金不得编造、HTML 必选章节、集合竞价报告模块（含 09:15/09:20/09:25 封单快照、通达信导出解析、同花顺/东方财富封单兜底顺序、竞价成交量爆量股量比 f50、竞价 HTML 无「数据源与可用性」）、金额亿/万格式、09:25:30 调度钩子、API。
